...
class ChallengeDetailsScreen3 extends StatefulWidget {
  final String title;
  final String date;
  final String amount;

  const ChallengeDetailsScreen3({
    Key? key,
    required this.title,
    required this.date,
...
class _ChallengeDetailsScreenState extends State<ChallengeDetailsScreen3> {
  ...
  Future<void> _loadCheckboxState() async {
    ...
    setState(() {
      _daysCompleted = List.generate(
        31,
            (index) =>
        _prefs.getBool('$_challengeKey/day_$index') ?? false,
      );
    });
  }
